...
ALTER TABLE p4coupon
	DROP CONSTRAINT PK_p4coupon; -- 쿠폰종류 기본키
...
-- 쿠폰종류 기본키
CREATE UNIQUE INDEX PK_p4coupon
	ON p4coupon ( -- 쿠폰종류
		cou_code ASC -- 쿠폰 코드
	);
...
ALTER TABLE p4coupon
	ADD
		CONSTRAINT PK_p4coupon -- 쿠폰종류 기본키
		PRIMARY KEY (
			cou_code -- 쿠폰 코드
		);
...
CREATE TABLE P4report (
	...
	report_curr    VARCHAR2(30)   DEFAULT '처리중' CONSTRAINT report_curr_ck CHECK (report_curr IN('처리중','반려처리','렌탈상품중지','사용자계정중지'))  -- 처리 상태
);
...
ALTER TABLE p4memcoupon
	DROP CONSTRAINT FK_member_TO_p4memcoupon; -- 회원 -> 보유쿠폰

-- 보유쿠폰
ALTER TABLE p4memcoupon
	DROP CONSTRAINT FK_p4coupon_TO_p4memcoupon; -- 쿠폰종류 -> 보유쿠폰
...
CREATE TABLE p4memcoupon (
	memcou_code  NUMBER       NOT NULL, -- 보유쿠폰 코드
	cou_code     NUMBER       NOT NULL, -- 쿠폰 코드
	mem_id       VARCHAR2(30) NOT NULL, -- 회원 ID
...
ALTER TABLE p4memcoupon
	ADD
		CONSTRAINT FK_member_TO_p4memcoupon -- 회원 -> 보유쿠폰
		FOREIGN KEY (
			mem_id -- 회원 ID
		)
		REFERENCES member ( -- 회원
...
ALTER TABLE p4memcoupon
	ADD
		CONSTRAINT FK_p4coupon_TO_p4memcoupon -- 쿠폰종류 -> 보유쿠폰
		FOREIGN KEY (
			cou_code -- 쿠폰 코드
		)
		REFERENCES p4coupon ( -- 쿠폰종류
			cou_code -- 쿠폰 코드
		)
		ON DELETE NO ACTION
		ON UPDATE NO ACTION;
...
CREATE TABLE P4qna (
	...
	mem_id      VARCHAR2(30)   NOT NULL, -- 회원 ID
	qna_cate    VARCHAR2(30)   NOT NULL CONSTRAINT qna_cate_ck CHECK (qna_cate IN ('계정문의', '차단/제재', '거래신고', '서비스기능', '광고전문상점', '서비스장애', '제안')), -- Q&A 카테고리
...
CREATE TABLE P4alarm (
	...
	mem_id      VARCHAR2(30) NOT NULL, -- 회원 ID
	rental_code NUMBER       DEFAULT 0,     -- 렌탈 코드
	report_code NUMBER       DEFAULT 0,     -- 신고 코드
	qna_code    NUMBER       DEFAULT 0,     -- Q&A 글 코드
	query_code  NUMBER       DEFAULT 0,     -- 문의 코드
	req_code    NUMBER       DEFAULT 0,     -- 렌탈요청 글 코드
	alarm_chk   NUMBER       DEFAULT 0 CONSTRAINT alrm_chk_ck CHECK (alarm_chk IN (0,1))  -- 확인여부
);
